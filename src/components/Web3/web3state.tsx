		...
		const initializeWeb3 = async () => {
			try {
				...
				if (window.ethereum) {
					...
					let accounts = await provider.send(
						'eth_requestAccounts',
						[]
					)
					let userAddress: string = accounts[0]
...
export async function mintSubCase(
	web3State: Web3State,
	caseNo: number,
	linkHash: string
) {
	...
	try {
		...
		// const caseNo = receipt.events?.[2].args?.[1].toNumber();
		console.log(
			'New subcase is added to caseNO with tokenId',
			caseNo,
			tokenId
		)
	} catch (error) {
...
export async function getCaseTokens(
	web3State: Web3State,
	caseNo: number
): Promise<number[]> {
	...
	try {
		...
		const tokenIds: number[] = tokens.map((tokenId: ethers.BigNumber) =>
			tokenId.toNumber()
		)
