...
import { useWeb3State } from './Web3/web3state'

const Dashboard = () => {
	const [open, setOpen] = React.useState<boolean>(false)
	const [count, setCount] = React.useState<number>(1)
	const [usercases, setUsercases] = React.useState<number[]>()
	...
	React.useEffect(() => {
	  const fetchcases = async()=>{
		if(contract && userAdd){
			let cases = await contract.getUserCases(userAdd);
	...
	console.log("cases of user ",userAdd, " are ", usercases);
				...
				<div className='flex flex-col gap-2'>
					...
					{usercases?.length == 0 ? (
						...
						<div className='relative h-[500px] w-full shadow-xl font-poppins justify-around flex-wrap rounded-md flex p-4 gap-4 overflow-y-scroll scrollbar-hide'>
							{usercases && usercases.map((caseItem, index) => (
							<div key = {index} className='h-96 w-72 rounded-3xl bg-[#E5E5E599]/60 px-3 py-3 flex flex-col overflow-hidden shadow-xl'>
